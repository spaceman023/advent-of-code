const fs = require('fs');
const input = fs.readFileSync('./inputs/7', 'utf-8').trim().split('\n');
const SPLIT_RE = /(?:\[|\])/;
...
console.log('Part 1:', input.filter(supportTLS).length);

console.log('Part 2:', input.filter(supportSSL).length);
